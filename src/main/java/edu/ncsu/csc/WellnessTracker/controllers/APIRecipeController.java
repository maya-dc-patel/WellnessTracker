...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( service.findAll().size() < 3 ) {
            if ( recipe.validateRecipe() == 0 ) {
                service.save( recipe );
                return new ResponseEntity( successResponse( recipe.getName() + " successfully created" ),
                        HttpStatus.OK );
            }
            else {
                if ( recipe.validateRecipe() == 1 ) {
                    return new ResponseEntity( errorResponse( "No ingredients in " + recipe.getName() ),
                            HttpStatus.I_AM_A_TEAPOT );
                }

                if ( recipe.validateRecipe() == 2 ) {
                    return new ResponseEntity( errorResponse( "Invalid price - must be a postivie integer." ),
                            HttpStatus.NOT_ACCEPTABLE );
                }

                return new ResponseEntity( errorResponse( "Ingredient  amount is too low for " + recipe.getName() ),
